...
class RadarNode
{
	...
	constructor(_ID:string, _Label:string, _Color:string, _Value:number, _Min:number, _Max:number)
	{
		this.ID = _ID;
		this.Label = _Label;
		this.Color = _Color;
	...
	GetScaledValue(radius: number): number
	{
		this.ScaleRatio = radius / (this.Max - this.Min);
		return this.Value * this.ScaleRatio;
	}
	SetValueFromScaledValue(valueScaled: number): number
	{
...
class Radar
{
	...
	constructor(params: RadarParams)
	{
		...
		var AngleRad = 0;
		this.Nodes.forEach(function (N: RadarNode)
		{
		...
		this.Canvas.onmousedown = function (ev: MouseEvent)
		{
			...
			R.Nodes.forEach(function (N: RadarNode)
			{
				var NodeX = N.Point.X + (R.ScalingDetails.x / R.ScalingRatio),
					NodeY = N.Point.Y + (R.ScalingDetails.y / R.ScalingRatio);
		...
		this.Canvas.onmousemove = function (ev: MouseEvent)
		{
			if (R.MouseTracking)
			{
				var MouseX = ev.offsetX / R.ScalingRatio,
					MouseY = ev.offsetY / R.ScalingRatio;

				var OriginX = MouseX - R.Center.X - (R.ScalingDetails.x / R.ScalingRatio),
					OriginY = MouseY - R.Center.Y - (R.ScalingDetails.y / R.ScalingRatio);
	...
	DrawWeb()
	{
		...
		for (var R: number = IncrementRadiusPx; R <= this.Radius; R += IncrementRadiusPx)
		{
			for (Theta = 0; Theta < (Math.PI * 2); Theta += IncrementAngleRad)
			{
	...
	DrawNodes()
	{
		...
		for (var i = 0; i < this.Num; ++i)
		{
			...
			n.Point = new RadarPoint
				(
					this.Center.X + Math.sin(n.AngleRads) * value_scaled,
					this.Center.Y + Math.cos(n.AngleRads) * value_scaled
				);
		...
		this.Nodes.forEach(function (n: RadarNode)
		{
			...
			R.Ctx.beginPath();
			var TxtPnt = new RadarPoint
			(
				R.Center.X + Math.sin(n.AngleRads) * labelRadius,
				R.Center.Y + Math.cos(n.AngleRads) * labelRadius
			);
	...
	scaleCanvas(): RadarPoint
	{
		var desiredSize:number = window.innerWidth * 0.9;
		if (desiredSize < 350)
			desiredSize = 350;
		...
		// For offsetWidth & offsetHeight to work in IE11, the element needs to be visible
		// So, let's pull it out of the DOM and put it somewhere visible, then put it back afterwards
		var parentDiv = this.Canvas.parentElement;
		var placeInDom = parentDiv.parentElement;

		document.body.appendChild(parentDiv);

		this.ScalingDetails = this.getObjectFitSize(
			true,
		...
		this.ScalingRatio = Math.min(
			this.Canvas.offsetWidth / originalWidth,
			this.Canvas.offsetHeight / originalHeight
		);
	...
	// Src: https://medium.com/@doomgoober/understanding-html-canvas-scaling-and-sizing-c04925d9a830
	getObjectFitSize(
